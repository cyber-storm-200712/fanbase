 ...
 * approved.
 * @property {Object} msgParams.metamaskId Added to msgParams for tracking and identification within Fanbase.
...
export default class EncryptionPublicKeyManager extends EventEmitter {
  ...
  addUnapprovedMessageAsync(address, req) {
    return new Promise((resolve, reject) => {
      if (!address) {
        reject(new Error('Fanbase Message: address field is required.'));
        return;
      }
          ...
          case 'rejected':
            reject(
              ethErrors.provider.userRejectedRequest(
                'Fanbase EncryptionPublicKey: User denied message EncryptionPublicKey.',
              ),
            );
            return;
          default:
            reject(
              new Error(
                `Fanbase EncryptionPublicKey: Unknown problem: ${JSON.stringify(
                  address,
                )}`,
              ),
            );
   ...
   * this.memStore.
   *
   * @param {Object} address - The param for eth_getEncryptionPublicKey call to be made after the message is approved.
   * @param {Object} [req] - The original request object possibly containing the origin
   ...
   *
   * @param {Object} msgParams The msgParams to be used when eth_getEncryptionPublicKey is called, plus data added by Fanbase.
   * @param {Object} msgParams.metamaskId Added to msgParams for tracking and identification within Fanbase.
