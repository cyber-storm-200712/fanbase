    ...
    this.readCryptoHDKeyOrCryptoAccount = () => {
      return new Promise((resolve, reject) => {
        ...
        this.on('keystone-sync_success-hdkey', (cbor) => {
          const cryptoHDKey = bcUrRegistryEth.CryptoHDKey.fromCBOR(
        ...
        this.on('keystone-sync_success-account', (cbor) => {
          const cryptoAccount = bcUrRegistryEth.CryptoAccount.fromCBOR(
            Buffer.from(cbor, 'hex'),
          );
          this.resetState();
    ...
    this.requestSignature = (signRequest, requestTitle, requestDescription) => {
      return new Promise((resolve, reject) => {
        ...
        this.once(`${requestId}-signed`, (cbor) => {
          const ethSignature = bcUrRegistryEth.ETHSignature.fromCBOR(
            Buffer.from(cbor, 'hex'),
          );
          this.resetState();
...
class MetaMaskKeyring extends baseEthKeyring.BaseKeyring {
  ...
  async signTransaction(address, tx$1) {
    const dataType =
      tx$1.type === 0
        ? bcUrRegistryEth.DataType.transaction
        : bcUrRegistryEth.DataType.typedTransaction;
    let messageToSign;
    ...
    const ethSignRequest = bcUrRegistryEth.EthSignRequest.constructETHRequest(
      messageToSign,
      dataType,
      hdPath,
      this.xfp,
