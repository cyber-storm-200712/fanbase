...
class Wallet {
  ...
  static generate(icapDirect = false) {
    if (icapDirect) {
      const max = new ethereumjs_util_1.BN(
  ...
  static async fromV1(input, password) {
    ...
    const derivedKey = await (0, scrypt_js_1.scrypt)(
    ...
    const mac = (0, ethereumjs_util_1.keccak256)(
    ...
    const decipher = crypto.createDecipheriv(
      'aes-128-cbc',
      (0, ethereumjs_util_1.keccak256)(derivedKey.slice(0, 16)).slice(0, 16),
  ...
  static async fromV3(input, password, nonStrict = false) {
    ...
    if (json.crypto.kdf === 'scrypt') {
      ...
      derivedKey = await (0, scrypt_js_1.scrypt)(
        Buffer.from(password),
   ...
   */
  getAddress() {
    return Promise.resolve((0, ethereumjs_util_1.publicToAddress)(this.pubKey));
  }
  /**
