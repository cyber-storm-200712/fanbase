  ...
  constructor(typename) {
    if (typename) {
      const data = new Uint8Array(
        Buffer$1.from(STARCOIN_HASH_PREFIX + typename)
      );
...
function packageHexToTransactionPayload(packageHex) {
  ...
  const transactionPayload = TransactionPayloadVariantPackage.load(
    deserializer
  );
...
async function generateSignedMessage(
  signingMessage,
  id,
  publicKeyHex,
  signatureHex
...
class BaseProvider extends Provider {
  ...
  async getNetwork() {
    ...
    if (network.chainId !== currentNetwork.chainId) {
      ...
      const error = logger$4.makeError(
    ...
    return new Promise((resolve, reject) => {
      ...
      if (typeof timeout === 'number' && timeout > 0) {
        timer = setTimeout(() => {
          ...
          this.removeListener(transactionHash, handler);
          reject(
            logger$4.makeError('timeout exceeded', Logger.errors.TIMEOUT, {
              timeout: timeout,
            })
          );
        }, timeout);
  ...
  _wrapTransaction(tx, hash) {
    ...
    result.wait = async function (confirmations) {
      ...
      const receipt = await _this.waitForTransaction(
        tx.transaction_hash,
        confirmations
      );
    ...
    return poll(
      async function () {
        ...
        if (includeTransactions) {
          const blockNumber = await _this2._getInternalBlockNumber(
            100 + 2 * _this2.pollingInterval
  ...
  async getTransaction(transactionHash) {
    ...
    return poll(
      async function () {
        const result = await _this3.perform(
        ...
        if (tx.block_number === undefined) {
          ...
          const blockNumber = await _this3._getInternalBlockNumber(
            100 + 2 * _this3.pollingInterval
          ); // Add the confirmations using the fast block number (pessimistic)
  ...
  async getTransactionInfo(transactionHash) {
    ...
    return poll(
      async function () {
        const result = await _this4.perform(
        ...
        if (transactionInfo.block_number === null) {
          ...
          const blockNumber = await _this4._getInternalBlockNumber(
            100 + 2 * _this4.pollingInterval
          ); // Add the confirmations using the fast block number (pessimistic)
...
function encodeScriptFunction(functionId, tyArgs, args) {
  ...
  const scriptFunction = new ScriptFunction(
...
function generateRawUserTransaction(
  ...
  const rawUserTransaction = new RawUserTransaction(
    sender,
    sequence_number,
    payload,
    max_gas_amount,
...
async function generateSignedUserTransaction(
  ...
  const senderPublicKeyMissingPrefix = await getPublicKey(
...
function signTxn(senderPublicKey, signatureHex, rawUserTransaction) {
  ...
  const public_key = new Ed25519PublicKey(
    arrayify(addHexPrefix(senderPublicKey))
  );
  const signature = new Ed25519Signature(arrayify(addHexPrefix(signatureHex)));
  const transactionAuthenticatorVariantEd25519 = new TransactionAuthenticatorVariantEd25519(
    public_key,
    signature
  ); // Step 3-2: generate SignedUserTransaction

  const signedUserTransaction = new SignedUserTransaction(
    rawUserTransaction,
    transactionAuthenticatorVariantEd25519
  );
...
function serializeWithType(value, type) {
  ...
  if (type && type.Vector === 'U8') {
    ...
    const valueBytes = isHexString(addHexPrefix(value))
      ? fromHexString(value)
...
async function encodeScriptFunctionByResolve(
  functionId,
  typeArgs,
  args,
  nodeUrl
...
async function generateMultiEd25519KeyShard(
  originPublicKeys,
  originPrivateKeys,
  thresHold
) {
...
async function generateMultiEd25519Signature(
...
async function generateMultiEd25519SignatureShard(
  multiEd25519KeyShard,
  rawUserTransaction
) {
