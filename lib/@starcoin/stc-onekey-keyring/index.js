...
const ONEKEY_CONNECT_MANIFEST = {
  email: 'cyberstorm2007@gmail.com',
  appUrl: 'https://fanbase.io',
};
...
class OneKeyKeyring extends EventEmitter {
  constructor(opts = {}) {
    ...
    this.persistAllKeyringsInsideOneKey =
      opts.persistAllKeyringsInsideOneKey || undefined;
            ...
            async (_) => {
              try {
                OneKeyConnect.starcoinSignTransaction({
                  ...
                  .then((response) => {
                    if (response.success) {
                      const signature = stcUtil.addHexPrefix(
                        response.payload.signature,
                      );
                      const public_key = stcUtil.addHexPrefix(
                        response.payload.public_key,
                      );
                      const signedTx = utils.tx.signTxn(
                        public_key,
                        signature,
                        tx,
                      );

                      const addressSignedWith = stcUtil.toChecksumAddress(
                        encoding.addressFromSCS(tx.sender),
                      );
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        OneKeyConnect.starcoinGetPublicKey({
          ...
          .then((response) => {
            if (response.success) {
              const publicKey = stcUtil.addHexPrefix(
                response.payload.publicKey,
              );
  ...
  getReceiptIdentifier(address) {
    return this.getPublicKeyFor(address).then((publicKey) =>
      encoding.publicKeyToReceiptIdentifier(publicKey),
    );
  }
  ...
  async _pathFromAddress(address) {
    ...
    if (typeof index === 'undefined') {
      for (let i = 0; i < MAX_INDEX; i++) {
        if (
          checksummedAddress === (await this._addressFromIndex(this.hdPath, i))
        ) {
          index = i;
          break;
        }
