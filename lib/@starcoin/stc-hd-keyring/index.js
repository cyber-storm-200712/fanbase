const hdkey = require('../stc-wallet/hdkey');
...
class HdKeyring extends SimpleKeyring {
  ...
  addAccounts(numberOfAccounts = 1) {
    ...
    const hexWallets = newWallets.map((w) => {
      return w.getAddress().then((address) => {
  ...
  exportAccount(address) {
    return this._getWalletForAccount(address).then((wallet) =>
      wallet.getPrivateKey().toString('hex'),
    );
  }

  signTransaction(address, tx, opts = {}) {
    return this._getWalletForAccount(address, opts).then((w) => {
  ...
  getPublicKeyFor(withAccount, opts = {}) {
    return this._getWalletForAccount(withAccount, opts).then((w) => {
