...
class SimpleKeyring extends EventEmitter {
  ...
  serialize() {
    return Promise.resolve(
      this.wallets.map((w) => ({
        privateKey: w.getPrivateKey().toString('hex'),
        publicKey: w.getPublicKey().toString('hex'),
      })),
    );
  }
    ...
    return new Promise((resolve, reject) => {
      try {
        this.wallets = keyPairs.map(({ privateKey, publicKey }) => {
          ...
          const wallet = Wallet.fromPrivatePublic(
            privateKeyBuffer,
            publicKeyBuffer,
          );
          return wallet;
  ...
  addAccounts(n = 1) {
    ...
    const hexWallets = newWallets.map((w) =>
      stcUtil.bufferToHex(w.getAddress()),
    );
  ...
  getAccounts() {
    return Promise.resolve(
      this.wallets.map((w) => stcUtil.bufferToHex(w.getAddress())),
    );
  }
  ...
  signTransaction(address, tx, opts = {}) {
    ...
    const hex = utils.tx.signRawUserTransaction(privKeyStr, tx);
  ...
  signMessage(address, data, opts = {}) {
    ...
    var rawMsgSig = stcUtil.bufferToHex(
  ...
  newGethSignMessage(withAccount, msgHex, opts = {}) {
    ...
    const rawMsgSig = stcUtil.bufferToHex(
      sigUtil.concatSig(msgSig.v, msgSig.r, msgSig.s),
    );
    return Promise.resolve(rawMsgSig);
  }
  ...
  signPersonalMessage(address, message, opts = {}) {
    const privKey = this.getPrivateKeyFor(address, opts);
    return utils.signedMessage
      .signMessage(message, privKey.toString('hex'))
      .then((payload) => {
  ...
  getPublicKeyFor(withAccount, opts = {}) {
    const privKey = this.getPrivateKeyFor(withAccount, opts);
    const privKeyStr = stcUtil.addHexPrefix(privKey.toString('hex'));
    const publicKey = privateKeyToPublicKey(privKeyStr);
    return Promise.resolve(publicKey);
  }
  ...
  removeAccount(address) {
    if (
      !this.wallets
        .map((w) => stcUtil.bufferToHex(w.getAddress()).toLowerCase())
    ...
    this.wallets = this.wallets.filter(
      (w) =>
        stcUtil.bufferToHex(w.getAddress()).toLowerCase() !==
        address.toLowerCase(),
    );
  }
  ...
  _getWalletForAccount(account, opts = {}) {
    ...
    let wallet = this.wallets.find(
      (w) => stcUtil.bufferToHex(w.getAddress()) === address,
    );
    if (!wallet)
