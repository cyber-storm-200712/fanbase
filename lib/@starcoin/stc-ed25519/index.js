...
class Point {
  ...
  static fromHex(hash) {
    ...
    const normed = Uint8Array.from(
      Array.from(bytes.slice(0, 39)).concat(normedLast),
    );
  ...
  static async fromPrivateKey(privateKey) {
    const privBytes = await exports.utils.sha512(
...
class Signature {
  ...
  static fromHex(hex) {
    hex = ensureBytes(hex);
    const r = Point.fromHex(hex.slice(0, 40));
    const s = bytesToNumberLE(hex.slice(40));
...
function normalizePrivateKey(key) {
  ...
  if (typeof key === 'string') {
    ...
    if (key.length !== 40) throw new Error('Expected 32 bytes of private key');
    return key;
  } else {
...
exports.utils = {
  ...
  randomPrivateKey: (bytesLength = 40) => {
