...
// var MASTER_SECRET = Buffer.from('secp256k1 seed', 'utf8');
var MASTER_SECRET = Buffer.from('ed25519 seed', 'utf8');
...
Object.defineProperty(HDKey.prototype, 'publicKey', {
  ...
  set: function (value) {
    // assert(value.length === 32, 'Public key must be 32 bytes.')
...
HDKey.prototype.derive = function (path) {
  ...
  entries.forEach(function (c, i) {
    ...
    console.log('Assert:', childIndex, HARDENED_OFFSET);

    assert(
      childIndex < HARDENED_OFFSET,
      `Invalid index: ${(childIndex, HARDENED_OFFSET)}`,
    );
