const { default: EthWallet } = require('../../ethereumjs-wallet');
const { default: EthHDKey } = require('../../ethereumjs-wallet/dist/hdkey');
const HDKey = require('../stc-hdkey');
...
EthereumHDKey.fromMasterSeed = function (seedBuffer) {
  ...
  return fromHDKey(EthHDKey.fromMasterSeed(seedBuffer));
};

EthereumHDKey.fromExtendedKey = function (base58key) {
  // return fromHDKey(HDKey.fromExtendedKey(base58key));
  return fromHDKey(EthHDKey.fromExtendedKey(base58key));
};
...
EthereumHDKey.prototype.derivePath = function (path) {
  return fromHDKey(this._hdkey.derivePath(path));
};
...
EthereumHDKey.prototype.getWallet = function () {
  return Wallet.fromPrivatePublic(
    this._hdkey._privateKey,
    this._hdkey._publicKey,
  );
};
